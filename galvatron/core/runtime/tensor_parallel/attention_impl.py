...
class FlashSelfAttentionVarlen(torch.nn.Module):
    """Flash Attention for variable length sequences (packed sequences).
    
    This class handles packed sequences with cu_seqlens for efficient batched processing.
    Input shape: (total_seq_len, num_heads, head_dim) - already flattened packed sequences
    """
    ...
    def forward(self, q, k, v, cu_seqlens=None, max_seqlen=None):
        ...
        assert cu_seqlens is not None and max_seqlen is not None, \
            "cu_seqlens and max_seqlen must be provided for varlen attention"
...
import torch
import torch.distributed as dist
from flash_attn.flash_attn_interface import _flash_attn_forward, _flash_attn_backward, _flash_attn_varlen_forward, _flash_attn_varlen_backward
...
def zigzag_ring_flash_attn_varlen_backward(
    ...
    assert causal == True, "zigzag ring is meaningless for causal=False"
    # Single RingComm + KV/dKV packing — same *step schedule* as dense zigzag for overlap.
    #
    # Option A (wire dtype): before torch.stack([k, dk], ...), cast dk/dv to original_dtype
    # (e.g. bf16) so the packed tensor stays low-precision and ring bandwidth does not double
    # (stack([bf16 k, fp32 dk]) would promote to fp32). Tradeoff: dK/dV are quantized for that
    # P2P hop only; after recv we cast back to float32 for in-place += dk_buffer (local state).
    # Unpacked fp32 dK/dV final return still goes through .to(q.dtype) at the end.
    kv_comm = RingComm(process_group)
...
class ZigzagRingFlashAttentionVarlen(torch.nn.Module):
    def __init__(self, attention_dropout, cp_group, cp_ranks, softmax_scale=None, causal=True):
        super().__init__()
        self.softmax_scale = softmax_scale
        self.attention_dropout = attention_dropout
