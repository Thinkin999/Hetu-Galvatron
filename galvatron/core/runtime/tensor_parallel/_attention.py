...
#__all__ 是 Python 中的一个特殊变量，用于定义模块的公共接口。它是一个包含字符串的元组，这些字符串表示模块中公开的函数、类和常量。
# 当模块被导入时，__all__ 中的内容会被导出，这样其他模块就可以访问这些内容。
# 在这个例子中，context_parallel 和 set_rotate_method 是两个公开的函数，因此它们被包含在 __all__ 中。
# 这意味着当这个模块被导入时，context_parallel 和 set_rotate_method 可以被其他模块访问。

#因果行为，推测可能是不一样的轮次有不一样的因果，传递来的kv不一样有的是全局，有的是跳过，有的是causal？
...
aten = torch.ops.aten#pytorch aten操作命名空间
...
def _is_causal_behavior(
    ...
    if i == 0:
        return _CausalBehavior.IS_CAUSAL#和自己计算

    source_rank = (rank - i) % world_size#计算源rank
    if source_rank < rank or _cp_options.enable_load_balance:
        return _CausalBehavior.NOT_IS_CAUSAL#不计算
    else:
        return _CausalBehavior.SKIP#跳过

#如果是异步集合通信张量就调用async否则不调用
