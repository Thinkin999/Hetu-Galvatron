...
def _get_sp_size_default(sp_option):
    """默认的 sp_size 提取函数，兼容旧格式（int）和新格式（tuple）"""
    if isinstance(sp_option, tuple):
        return sp_option[0]
    return sp_option

def generate_balanced_initial_solution(model, sp_options, M, m, A, buckets, K, P, sp_size, hide_output, device_token_capacity, costmodel, get_sp_size=None):
    """
    生成平衡的初始解。
    
    Args:
        get_sp_size: 可选的辅助函数，用于从 sp_option 中提取 sp_size。
                     如果为 None，则使用默认函数兼容两种格式。
    """
    if get_sp_size is None:
        get_sp_size = _get_sp_size_default
    
    # 检查是否有匹配的 sp_size
    has_matching_sp = any(get_sp_size(opt) == sp_size for opt in sp_options)
    if not has_matching_sp:
        return
    ...
    active_groups = []
    for p in range(P):
        if get_sp_size(sp_options[p]) == sp_size:
