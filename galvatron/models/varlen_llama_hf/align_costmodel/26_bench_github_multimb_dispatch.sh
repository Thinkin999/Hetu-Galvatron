#!/usr/bin/env bash
# Dispatch a github multi-microbatch validation benchmark.
#
# Goal: for a realistic long-tail sequence-length distribution (github), compare
# four parallel-strategy configurations and verify cost-model predicted_speedup
# matches measured_speedup, both at small chunks (single mb per step) and large
# chunks (many mb per step → exercises the b-decomposition).
#
# Configurations (all at the same GBS, same dataset chunks of `seq_length`):
#   - ulysses:8                     (sp=8, cp=1)
#   - ring:8                        (sp=1, cp=8)
#   - usp:2x4                       (sp=2, cp=4)
#   - AdaCPSP (no forced strategy)  (solver picks per-microbatch)
#
# Each forced configuration is run twice (chunks=1, chunks=8) to exercise the
# multi-microbatch path. AdaCPSP runs at chunks=auto (solver decides).
#
# Encoding: cells = "<config>:<chunks>".
...
# ---------- run identity ----------
export GHMB_RUN_ID="${GHMB_RUN_ID:-ghmb_$(date +%Y%m%d_%H%M%S)}"
# Each cell = "<config>:<chunks>" where config ∈ {ulysses8, ring8, usp2x4, adacpsp}
# and chunks ∈ {1, 8, auto}.  We deliberately keep this conservative — 7 cells.
...
# global_train_batch_size — same for all forced configs (must be a multiple of
# num_groups × chunks). With num_groups=2 (16/8) and max chunks=8 → GBS=16.
export GHMB_GBS="${GHMB_GBS:-16}"
# Sequence-length cap: github has very long tail (max=779k); we cap to 65536
# so the filtered subset is large but still memory-feasible across all configs.
...
echo "==== Dispatching github multi-mb benchmark ${GHMB_RUN_ID} ===="
...
export NODE_RANK=1
mkdir -p ${LOG_DIR} ${PROFILE_DIR}
nohup setsid bash '${SCRIPT_DIR}/26_bench_github_multimb_worker.sh' \
    > '${W1_LOG}' 2>&1 < /dev/null &
echo \$! > '${LOG_DIR}/w1.pid'
"
