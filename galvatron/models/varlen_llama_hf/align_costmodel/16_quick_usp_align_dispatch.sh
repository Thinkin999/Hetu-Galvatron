#!/usr/bin/env bash
# 2-node fresh USP attention layer benchmark across (sp,cp) x seq grid.
# Reuses setsid/nohup + gpu_busy lifecycle.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

QUICK_RUN_ID="${QUICK_RUN_ID:-quick_usp_$(date +%H%M%S)}"
CASES="${CASES:-2x8,4x4,8x2,2x4,4x2,2x2}"
...
cleanup_zombies() {
    set +e
    pkill -9 -f '16_quick_usp_align.py' 2>/dev/null
    pkill -9 -f "torchrun.*${MASTER_PORT}" 2>/dev/null
    ssh "${WORKER1_HOST}" "pkill -9 -f '16_quick_usp_align.py' 2>/dev/null; \
...
echo "[dispatch] ${QUICK_RUN_ID}  cases=${CASES}  seqs=${SEQS}  num_seqs=${NUM_SEQS}"
stop_gpu_busy
cleanup_zombies
...
make_cmd() {
    ...
    echo "${TORCHRUN} \
      ...
      --node_rank ${node_rank} \
      ${SCRIPT_DIR}/16_quick_usp_align.py \
      --cases ${CASES} --seqs ${SEQS} --num-seqs ${NUM_SEQS} \
