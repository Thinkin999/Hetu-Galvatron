#!/usr/bin/env bash
# Worker-side launcher for ghmb ZeRO-2 benchmark.
#
# DIFFERS FROM 26_bench_github_multimb_worker.sh in ONE LINE:
#   --sdp 0  (was --sdp 1)
# Because parallel.py:65 has:
#   fsdp_type_dict = {0: get_args().default_dp_type, 1: "zero3"}
# so --sdp 1 *forces* ZeRO-3 regardless of --default_dp_type. To actually
# get ZeRO-2 we must set --sdp 0 AND --default_dp_type zero2.
#
# Why ZeRO-2? It eliminates per-microbatch param all-gather, removing the
# FSDP straggler effect that was polluting our cost model validation. The
# cost model "max_g(group_time) + sum_mb" formula then matches runtime
# wall-clock cleanly.
...
for cell in ${GHMB_CELLS}; do
    ...
    if [ "${forced_strategy}" = "UNKNOWN" ]; then
        log "GHMB-Z2 SKIP unknown config ${cfg}"
        continue
    fi
    ...
    log "GHMB-Z2 cell ${cell_label}  strategy='${forced_strategy:-auto}' chunks=${chunks}  NODE_RANK=${NODE_RANK}  [ZeRO-2: --sdp 0 --default_dp_type zero2]"
    ...
    $(torchrun_prefix) "${MODEL_DIR}/train_dist_adacpsp.py" \
        ...
        --dataset github \
        || log "GHMB-Z2 cell ${cell_label} EXITED (rc=$?) — continuing sweep"
done

log "GHMB-Z2 benchmark worker done."
