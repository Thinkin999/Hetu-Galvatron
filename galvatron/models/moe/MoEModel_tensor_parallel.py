...
from galvatron.core.runtime.tensor_parallel.attention import Attention, SelfAttentionSubmodules
...
class MoEAttention_tp(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, rotary_embedding):
        ...
        return hidden_states, mlp_residual, probs, routing_map

# TODO: Add shared expert support
class MoEMLP_tp(nn.Module):
    def __init__(self, config, tp_group=None, ep_group=None, tp_of_ep_group=None, tp_and_ep_group=None):
        super().__init__()
        args = get_args()
        megatron_config = core_transformer_config_from_args(args)
        self.config = megatron_config
        self.expert_parallel_size = mpu.get_expert_model_parallel_world_size(ep_group)
        assert self.expert_parallel_size > 0, "Expected non-negative expert parallel size"

        assert self.config.num_moe_experts % self.expert_parallel_size == 0
        self.num_local_experts = self.config.num_moe_experts // self.expert_parallel_size
        local_expert_indices_offset = (
            mpu.get_expert_model_parallel_rank(ep_group) * self.num_local_experts
        )
        ...
        if args.moe_grouped_gemm:
            self.experts = GroupedMLP(self.num_local_experts, config, tp_of_ep_group)
        else:
...
class MoELayer_tp(nn.Module):
    def __init__(self, config, layer_number, tp_group=None, sp_group=None, ep_group=None, tp_of_ep_group=None, tp_and_ep_group=None):
        ...
        self.mlp = MoEMLP_tp(config, tp_group, ep_group, tp_of_ep_group, tp_and_ep_group)
    ...
    def forward(
        ...
        attention_output, mlp_residual, probs, routing_map = self.attention(
            hidden_states,
            attention_mask,
            rotary_embedding,
        )
